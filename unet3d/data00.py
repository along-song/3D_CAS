...
def create_data_file(out_file, n_channels, n_samples, image_shape):
    ...
    # data_shape = tuple([0, n_channels] + list(image_shape[0]))   #(0,1,512,512,220)
    truth_shape = tuple([0, 1] + list(image_shape))
    data_storage = hdf5_file.create_earray(hdf5_file.root, 'data', tables.Float32Atom(), shape=data_shape,
                                           filters=filters, expectedrows=1)
    ...
    for data in range(1, n_samples):
        data_shape = tuple([0, n_channels] + list(image_shape[data]))  # (0,1,512,512,220)
        truth_shape = tuple([0, 1] + list(image_shape[data]))
        data_storage = hdf5_file.create_earray(hdf5_file.root, 'data', tables.Float32Atom(), shape=data_shape,
                                               filters=filters, expectedrows=1)
        truth_storage = hdf5_file.create_earray(hdf5_file.root, 'truth', tables.UInt8Atom(), shape=truth_shape,
                                                filters=filters, expectedrows=1)
...
def write_image_data_to_file(image_files, data_storage, truth_storage, image_shape, n_channels, affine_storage,
    ...
    for i,set_of_files in enumerate(image_files):
        print(i)
        images = reslice_image_set(set_of_files, image_shape[i], label_indices=len(set_of_files) - 1, crop=crop)
...
def write_data_to_file(training_data_files, out_file, training_data_files_shape, truth_dtype=np.uint8, subject_ids=None,
                       normalize=True, crop=True):
    """
    ...
    """
    n_samples = len(training_data_files)  #the number of samples
    n_channels = len(training_data_files[0]) - 1          #1
    try:
        # for i in range(n_samples):
        #     shape=training_data_files_shape[i]
        hdf5_file, data_storage, truth_storage, affine_storage = create_data_file(out_file,
                                                                                  n_channels=n_channels,
                                                                                  n_samples=n_samples,
                                                                                  image_shape=training_data_files_shape)

        # print(affine_storage)
    except Exception as e:
        # If something goes wrong, delete the incomplete data file
        os.remove(out_file)
        raise e

    write_image_data_to_file(training_data_files, data_storage, truth_storage, training_data_files_shape,
