...
def patch_wise_prediction(model, data, overlap=0, batch_size=1, permute=False):
    ...
    while i < len(indices):
        ...
        for predicted_patch in prediction:
            #predicted_patch=gaussian_filters(predicted_patch,1,0)
            predictions.append(predicted_patch)
    #output_shape = [int(model.output.shape[1])] + list(data.shape[-3:])       #signle loss used
    output_shape = [int(model.output[0].shape[1])] + list(data.shape[-3:])   #multi loss used
    return reconstruct_from_patches(predictions, patch_indices=indices, data_shape=output_shape)

def  gaussian_filters(deal_patch,center,r):
     processed_patch = ndimage.gaussian_filter(deal_patch,sigma=center, order=0)
     #processed_patch=cv2.GaussianBlur(deal_patch,ksize=center,sigmaX=r)
